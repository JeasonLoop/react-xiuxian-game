...
### 修正與優化
- 創角頁的戰鬥稟賦改為同時顯示「投入點數」與角色面板使用的「開局實值」。
- 創角預覽與角色初始化共用同一套換算公式，並納入天生異能帶來的被動數值加成。
- 世界存檔保留創角投入點，角色面板可對照原始投入與目前實際能力。
- 修正悟性、心性投入 0 點時，進入遊戲後被暗中改成 1 點的問題。
...
### 修正與優化
- 縮窄模板行動的接管範圍，不再把一般的「看看／查看」自由輸入都套入固定模板。
- 「看看此地有哪些修士」「查看四周有哪些人物可結交」改為精確的在地人物查詢，不耗費遊戲時間。
- 「環視四周」與「打探天下消息」分流，前者查看眼前人物，後者才顯示世界事件與風聲。
- 在地人物改採具體地點比對，避免把同一大區、實際身在他處的角色錯列為眼前人物。
