...
const TalentCard: React.FC<TalentCardProps> = ({ talent, selected, canAfford, onToggle }) => {
  ...
  return (
    <div
      onClick={handleClick}
      className={`relative flex min-h-[126px] flex-col p-3 rounded-lg border-2 transition-all duration-200 select-none ${
        selected
    ...
    >
      ...
      <div className="mb-2 flex items-start justify-between gap-2 pr-6">
        <div className="flex min-w-0 items-start gap-1.5">
          <span className={`break-words text-sm font-bold leading-tight ${getRarityTextColor(talent.rarity)}`}>
            {talent.name}
          </span>
          {talent.specialAbility && (
            <Star size={13} className="mt-0.5 flex-shrink-0 fill-amber-400 text-amber-400" />
          )}
        </div>
        <span className="flex-shrink-0 rounded border border-amber-500/40 bg-stone-950/60 px-1.5 py-0.5 text-[10px] font-bold text-amber-300">
          {talent.fateCost}点
        </span>
      </div>
      ...
      {statEntries.length > 0 && (
        <div className="mt-auto grid grid-cols-1 gap-x-2 gap-y-0.5 border-t border-stone-700/50 pt-1.5 min-[420px]:grid-cols-2">
          {statEntries.map(([stat, value]) => {
            const Icon = STAT_ICONS[stat] || Sparkles;
            return (
              <div key={stat} className="flex min-w-0 items-center gap-1">
