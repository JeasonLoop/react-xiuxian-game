...
### 🎉 新增功能
- **九天通天塔**: 100 重天挑战玩法
  - 进入秘境探索后可挑战守塔者，层层递进不可跨层
  - 首通奖励：修为、灵石、太虚洗炼石与太虚悟道卷
  - 已破关层可重挑扛炼但不再发放首通奖励
  - 每日一次扫荡，按历史最高层结算次级物资
- **自创神通**: 金丹期后可融汇两门功法
  - 槽位随境界增加（金丹 1 格，长生 5 格），炼气/筑基期正确显示 0 格
  - 可自命名或由道蕴自动命名，最高升至第十重
  - 暴击、减伤、闪避、吸血等效果会进入回合制与快速战斗
- **万宝仙炉·装备洗炼**: 洞府中使用太虚洗炼石重铸装备词条
  - 品质越高词条数与上限越高，可决定攻击/防御/气血/暴击/闪避/吸血
- **灵兽远征**: 洞府可派遣灵兽出外寻珍
  - 队伍数随洞府等级解锁，探索完成后领取灵石、修为与洗炼石

### 🐛 修复
- **交易行购买**: 联机购买改为先推存档再确认，避免重复扣石与自动存档回滚
- **售卖结算**: 领取灵石时先检查存档，写入失败回滚已领取状态
- **自创神通写回**: 熔炼/升级/遗忘仅更新灵石、背包与神通列表，不覆盖同期修炼进度
- **通天塔/洞府写回**: 挑战、扫荡、洗炼与远征结算改为差量写回，避免整包覆盖玩家存档
- **首通奖励**: 重挑已破层时不再重复发放首通奖励

### 🔧 优化改进
- **战斗计算**: 自创神通与洗炼词条接入回合制与快速结算
- **物品入包**: 通天塔首通/扫荡改用统一入包逻辑，材料类物品正确叠加
- **神通消耗**: 升级灵石消耗收口为统一常量函数，界面与结算一致
...
### 🔧 优化改进
- **功能开放节奏调整**: 移除境界功能解锁限制，核心系统默认全开放，减少前期等待成本。
- **历练战斗流程优化**: 手动历练恢复常驻回合制战斗弹窗，自动历练保留快速结算，普通攻击不再误触发跳过战斗。
- **灵宠战斗平衡**: 灵宠重新参与手动战斗，并为灵宠单次伤害增加合理上限，保留存在感的同时避免一击秒杀导致战斗过早结算。
- **战斗可读性优化**: 手动战斗与快速战斗日志新增出手来源、伤害标签和战斗统计，便于观察本体、灵宠、敌方的实际输出表现。
- **游戏时长统计优化**: 游戏时长改为按前台活跃时间累计，避免后台挂起、电脑休眠或旧存档写入导致统计偏差。
- **秘境与成长数值微调**: 降低被动回血速度、下调关键词历练功法解锁概率，并优化秘境奖励区间，放缓资源和战力膨胀。
- **开局体验保留**: 新角色继续保留开局十连抽，保证前期构筑选择空间。

### 🐛 修复
- **灵兽秘径提示**: 灵石、气血或修为不足时，复用全局右上角轻提示反馈失败原因，避免点击后无明显反馈。
- **灵宠战斗日志**: 战斗回放中灵宠行动统一标记为「灵宠出手」，降低日志理解成本。

## [0.3.6] - 2026-06-23

### 🎉 新增功能
- **交易行系统**: 全新的玩家交易市场
  - 支持上架物品到市场，其他玩家可浏览和购买
  - NPC 常驻商货：24 种稀有/传说/仙品物品常驻出售
  - 玩家上架：可将背包中的物品设定价格挂单出售
  - 本地 + 云端双模式：本地存档独立运行，联网后可通过 Cloudflare Worker KV 实现跨玩家交易
  - 购买确认机制：防止误操作，高价物品需二次确认
  - 入口：主界面交易行按钮
- **难度模式系统**: 多档难度选择，影响战斗与收益
  - 简单/普通/困难/地狱 四档难度
  - 难度影响敌人属性、战斗奖励倍率
  - 高难度获得更多奖励，适合追求挑战的玩家
- **灵兽秘径代价机制**: 进化材料获取不再免费
...
### 🐛 修复
- **购买回滚**: 交易行购买确认失败时，正确回滚已扣除的灵石和物品
- **市场分页**: 修复市场物品列表分页失效问题，正确加载全部商品
- **Worker 冷启动**: 修复 Cloudflare Worker 冷启动时市场数据未初始化的问题
- **成就过滤**: 修复成就列表未正确区分全局/本地成就的问题
- **Linux.DO 登录**: 修复 OAuth 授权 URL 常量缺失导致的编译错误

### 🔧 优化改进
- **经验曲线重构**: 全面重构升级经验公式，升级节奏更加合理
  - 新增 `getRealmMaxExp` 函数，统一计算各境界各层经验需求
  - 基础经验需求提升约 2.4 倍，延长每个境界的修炼周期
  - 层间经验增长系数 0.24，大境界间跳跃更平滑
- **装备强化曲线下调**: 降低祭炼升级的属性成长率，避免后期属性膨胀
- **丹药永久属性递减**: 使用丹药获得永久属性加成引入递减机制，防止无限堆叠
- **战斗奖励调整**: 降低战斗奖励倍率，配合经验曲线控制资源产出
- **灵宠战斗增强**: 提升灵宠在战斗中的辅助效果，增强灵宠培养价值
- **离线收益优化**: 改进离线收益结算逻辑，与新版经验曲线兼容
...
### 🎉 新增功能
- **天赋选择系统**: 创建角色时可自由选择天赋，命运点预算制玩法
  - 新增 5 大天赋类别：体质、悟性、战斗、气运、特殊，共 30+ 种天赋
  - 天赋分为普通/稀有/传说/仙品 4 个稀有度，稀有度越高消耗命运点越多
  - 命运点预算制：玩家在点数限制内自由搭配天赋组合，打造专属 Build
  - 支持按类别筛选、随机分配、清空重选等便捷操作
  - 天赋提供初始属性加成，影响角色开局方向
  - 入口：开始游戏 → 创建角色界面
- **Linux.DO 账号登录**: 支持通过 Linux.DO 社区账号快捷登录
  - OAuth 2.0 授权流程，安全便捷，无需额外注册
  - 支持弹窗授权和回调 URL 两种回退方案
  - 登录界面新增「通过 Linux.DO 登录」按钮
- **Cloudflare 一键部署**: 新增 `scripts/deploy-cf.mjs` 部署脚本
  - 支持一键将前端部署到 Cloudflare Pages，后端部署到 Cloudflare Workers
  - 自动加载 `.env` 和 `.env.production` 环境变量
  - 替代原有 `deploy.sh` 脚本

### 🐛 修复
- **Worker API**: 修复 Cloudflare Worker 接口调用异常

### 🔧 优化改进
- 设置弹窗新增 Linux.DO 友情链接入口
- 登录界面优化：Linux.DO 快捷登录与传统登录并存
- 部署流程简化：移除旧 `deploy.sh`，统一为 Node.js 脚本
...
### 🎉 新增功能
- **NPC 社交系统**: 完整的人物志与人际关系玩法
  - 新增人物志界面，展示所有结识的修仙者，7 级好感度体系（道侣/好友/友善/中立/冷淡/敌视/仇敌）
  - 支持赠送物品或灵石提升好感度，好感度等级带来实际 buff（修炼速度、灵石获取加成）
  - 道侣（好感≥80）在战斗中几率助阵，仇敌（好感≤-50）可能在冒险中追杀
  - 入口：左侧 StatsPanel 人际关系区域点击
- **转世重修系统**: 终局 Prestige 循环
  - 元婴期 9 层经验满后可选择转世重修，重置境界但保留真灵之力
  - 5 级轮回（初转→五转圆满），全属性 +10%~70%，修炼速度 +10%~70%
  - 保留灵根（强化）、部分功法、所有灵宠、成就记录
  - 入口：角色面板 → 传承区域
- **秘境 Roguelike 地宫**: 多层随机探索玩法
  - 5-12 层随机地宫，每层 3 选 1 路径（战斗/宝藏/回复/奇遇/商人/Boss）
  - 每 5 层 Boss 战，奖励随层数递增，可随时安全退出
  - 入口：秘境探索界面顶部
- **功法羁绊套装**: Build 流派系统
  - 10 个功法羁绊套装（火系三绝/金系剑道/土系铁壁/水系灵动/木系长生/五行归元/剑意通神/混沌不朽/龙凤呈祥/风雷双绝）
  - 3 条 Build 路线（暴击/续航/反击），各有 4 级阈值奖励
  - 功法界面展示激活/可激活羁绊及流派进度
- **境界解锁系统**: 新手友好的功能渐进开放
  - 炼气期仅开放核心循环，筑基期解锁宗门/宠物/抽奖/功法，金丹期解锁秘境/炼丹/洞府/日常
  - 锁定功能灰化显示锁图标和解锁条件，突破时自动提示新解锁功能
- **一键装备**: 自动遍历 15 个装备槽位，装备背包中战力最高的装备
- **批量分解**: 一键将未装备的普通/稀有品质装备分解为炼器石
- **装备套装效果**: 同品级装备数量达标触发套装加成（4 传说 +10%、6 传说 +15%、4 仙品 +20%、8 稀有+ +8%）
- **突破感悟积累**: 突破失败不再纯看脸，每次失败积累"感悟"提升下次成功率（大境界 +5%/次，小境界 +3%/次，上限 +30%）
- **离线收益结算**: 离线超 30 秒后，上线自动结算修炼经验、灵石收入、洞府灵草收益（上限 24 小时）
- **新手目标提示**: GameHeader 显示当前修炼目标（如下一境界）

### 🐛 修复
- **天劫 x 自动历练**: 自动历练中触发天劫时立即停止，天劫完成后清除暂停状态
- **天劫境界错乱**: 刷新页面后 exp 被 clamp 导致 targetRealm 回退，金丹天劫误显示为筑基天劫
- **背包滚动**: 物品网格缺失 overflow-y-auto 导致底部物品被裁切；筛选区域过大压缩物品可视区——改为折叠式筛选
- **法宝数值**: 祭炼界面属性值统一 Math.floor() 格式化，消除浮点小数

### 🔧 优化改进
- **筛选区折叠**: 背包分类标签紧凑化，品质/属性/排序等收进可折叠筛选面板，默认只显示搜索框+分类标签
- **物品卡片紧凑化**: padding、图标、按钮尺寸缩小，3 列网格（大屏），同空间可显示 ~3 倍物品
- **人物志入口**: StatsPanel 人际关系区域可点击打开完整 NPC 列表
- **物品锁定功能**: 物品新增 🔒 锁定/解锁，锁定后不可丢弃、分解、出售、喂养、炼丹、炼器、赠送
- **批量分解弹窗**: 批量分解改为弹窗选择界面，按品质筛选、多选装备，预览炼器石产出
- **人物入口按钮**: GameHeader 新增 👥 人物按钮
- **成就提前解锁**: 成就系统从元婴期改为筑基期解锁

### 🐛 修复
- **天劫 x 自动历练**: 自动历练中触发天劫时立即停止
- **天劫境界错乱**: 刷新页面后 exp clamp 导致天劫显示错误境界
- **背包滚动**: 物品网格缺 overflow-y-auto 导致底部裁切；筛选区改为折叠式
- **法宝数值**: 祭炼界面属性值 Math.floor() 消除浮点小数
- **Token 过期登录**: 修复重新登录成功却提示"登录已过期"的竞态条件
- **跳过战斗二次确认**: 移除回合制战斗「跳过战斗」的 showConfirm 弹窗
- **日志上限**: 日志达上限后提示清理，新增清空按钮
- **NPC 遭遇率**: 提升 cultivator/rescue 事件频率，enlightenment/spiritSpring 新增偶遇 NPC

### 🔧 优化改进
- **筛选区折叠**: 品质/属性/排序等收进可折叠面板，默认只显示搜索框+分类标签
- **背包分类切换**: getItemCategory 提至模块级，物品卡片加 CSS containment 优化重绘
...
### 🎉 新增功能
- **经济平衡（境界物价）**：商店购买价与回收价按境界/层数动态缩放，形成更稳定的中后期资源消耗曲线。
- **抽奖软保底**：新增每累计 50 抽触发一次传说/仙品池保底。
- **重复灵宠补偿**：抽到已拥有灵宠（同物种）时，自动折算为灵石补偿，减少重复收益挫败感。

### 🐛 修复
- **每日任务进度修复**：修复秘境进入失败、宗门任务失败时仍可能错误增加进度的问题。
- **异步任务装饰器修复**：任务进度装饰器支持异步流程，并在 handler 返回 `false` 时不再记进度。
- **洞府自动收获一致性修复**：
  - 自动收获现在正确应用变异产量加成；
  - 自动收获会同步更新灵草图鉴并触发图鉴奖励结算；
  - 自动收获在启用/挂载后会立即执行一次检查，减少“需要等待轮询”的体验问题。
- **聚灵阵改造修复**：同一改造项不可重复叠加，避免材料循环投入导致异常增益。

### 🔧 文档与版本
- 项目版本更新至 `0.3.3.1`。
- 同步更新版本说明与文档索引中的更新日志引用路径。
...
### 💥 破坏性变更
- **完全移除AI依赖**: 删除所有AI相关代码、配置和第三方依赖
  - 删除 `config/aiConfig.ts` - AI配置模块
  - 删除 `api/proxy.js` - Vercel AI代理
  - 从 `package.json` 移除 `@google/genai` 依赖
  - 清理 `vite.config.ts` 中的AI代理配置
  - 清理所有环境变量中的 `VITE_AI_*` 配置项
  - 项目现在**完全离线**，所有历练事件使用内置模板库，不依赖任何外部AI服务

### 🎉 新增功能
- **JWT密钥自动生成**: 后端部署时如果未设置 `JWT_SECRET` 环境变量，会自动生成一个64字节的随机密钥，无需手动生成
- **一键部署支持**: 新增 `deploy.sh` 一键部署脚本，支持缓存依赖，仅在依赖变化时重新安装

### 🔧 配置更新
- 配置域名 `ylxiuxian.jeasonloop.online` 到 `nginx.conf`
- 更新 `docker-compose.full.yml` 默认 `VITE_API_BASE_URL` 为 `/api`（适配nginx代理）
- 更新所有环境变量示例，移除AI相关配置
- 修复Vercel配置，支持通用API代理

### 🔒 安全更新
- 改进JWT密钥处理逻辑，生产环境不再允许默认硬编码密钥，未设置时自动生成安全随机密钥
- 保持原有安全机制：SQLite数据持久化、环境变量隔离
...
### 游戏系统
- [x] 更多秘境探索内容 → 秘境 Roguelike 地宫
- [x] 灵宠进阶系统
- [x] 宗门任务系统
- [x] 社交功能（好友、聊天）→ NPC 社交系统
- [ ] 跨服/多人副本
- [ ] 装备附魔/铭文系统
