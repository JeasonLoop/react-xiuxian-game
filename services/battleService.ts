...
const createEnemy = async (
  ...
  return {
    ...
    maxHp: Math.max(
      40,
      Math.round(player.maxHp * (0.5 + Math.random() * 0.4) * finalDifficulty)
    ), // 从 0.75-1.25 降到 0.5-0.9
    speed: Math.max(
      6,
      Math.round(
        (player.speed || 10) * (0.7 + Math.random() * 0.3) * strengthMultiplier
      )
    ), // 从 0.8-1.2 降到 0.7-1.0
